...
class denied extends config{
    ...
    public function DeniedRemarks(){
        $config = new config();
        $con = $this->con();
        $sql = "UPDATE `tbl_client_user` SET `status` = 'DECLINED', `date_completed` = CURRENT_TIMESTAMP,  `remarks` = '$this->remarks' WHERE `id` = '$this->id'";
        ...
        sendDeniedEmail($lastname, $fullname, $status, $email, $remarks);
    }
}
?>
